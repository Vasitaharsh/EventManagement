...
class EventService {
  ...
  async payForEvent(eventId: Types.ObjectId, userId: Types.ObjectId, amount: number): Promise<IPayment> {
    const event = await Event.findById(eventId);
...
console.log(event);
    ...
    if (!event.paidStatus || !event.price || event.price !== amount) {
    ...
    event.payments.push(payment._id as Types.ObjectId);
    await event.save();
